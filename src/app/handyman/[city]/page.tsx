...
import {
  ...
  isPublishedLocalPage,
  type LocalCityKey,
} from "../../../lib/localPages";
import { createPageMetadata } from "../../../lib/seo";

const TRADE = "handyman" as const;
export const dynamicParams = false;

type PageProps = { params: Promise<{ city: string }> };

function resolveCity(value: string): LocalCityKey | null {
  return value in LOCAL_LOCATIONS ? (value as LocalCityKey) : null;
}

export function generateStaticParams() {
  return PUBLISHED_LOCAL_PAGES
    .filter((page) => page.trade === TRADE && page.city !== "birmingham")
    .map((page) => ({ city: page.city }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    openGraphDescription: content.webPageDescription,
    twitterDescription: content.webPageDescription,
  });
}

export default async function HandymanCityPage({ params }: PageProps) {
  const { city: value } = await params;
  const city = resolveCity(value);
