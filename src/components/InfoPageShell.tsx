...
export function InfoPageShell({eyebrow,title,intro,breadcrumbItems,summaryTitle,summary,children}:{eyebrow:string;title:string;intro:string;breadcrumbItems:BreadcrumbItem[];summaryTitle:string;summary:ReactNode;children:ReactNode;}){return <div className="local-trade-page"><Breadcrumbs items={breadcrumbItems}/><main id="top"><section className="seo-hero"><div className="wrap narrow"><span className="smallcap">{eyebrow}</span><h1>{title}</h1><p>{intro}</p></div></section><section className="answer-strip"><div className="wrap narrow answer-box info-answer-box"><h2>{summaryTitle}</h2><div>{summary}</div></div></section>{children}</main></div>}
